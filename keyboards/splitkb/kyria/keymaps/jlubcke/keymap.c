...
const uint16_t PROGMEM encoder_map[][NUM_ENCODERS][NUM_DIRECTIONS] = {
    ...
    [_LOWER]  = { ENCODER_CCW_CW(KC_PMNS,   KC_PPLS ), ENCODER_CCW_CW(MS_WHLU, MS_WHLD  ) },
...
double g(double x, double y, double p) {
    p = fmod(p, 1.0);
...
static void render_anim(void) {
    if (timer_elapsed(anim_timer) > FRAME_TIMEOUT) {
        ...
        if (is_oled_on()) {
            if ( ! oled_was_on) {
                selected_anim = (selected_anim + 1) % 2;
            }
            oled_was_on = 1;
        } else {
            oled_was_on = 0;
